...
namespace salary_calculate
{
    public partial class FormSalaryCalculator : Form, IReminderObserver
    {
        ...
        private double GetFamilyStatusCoefficient()
        {
            ...
            // Yeni numericUpDown kontrolleri ile çocuk yaş gruplarını hesapla
            if (hasChildrenCheckBox.Checked)
            {
                // 0-6 yaş arası çocuklar (numericUpDown1)
                int childrenUnder6 = (int)numericUpDown1.Value;
                if (childrenUnder6 > 0)
                {
                    coefficient += 0.20 * childrenUnder6;
                }

                // 7-18 yaş arası çocuklar (numericUpDown2)
                int children7to18 = (int)numericUpDown2.Value;
                if (children7to18 > 0)
                {
                    coefficient += 0.30 * children7to18;
                }

                // 18 yaş üstü çocuklar (numericUpDown3)
                int childrenOver18 = (int)numericUpDown3.Value;
                if (childrenOver18 > 0)
                {
                    coefficient += 0.40 * childrenOver18;
                }
            }
        ...
        private void checkBoxHasChildren_CheckedChanged(object sender, EventArgs e)
        {
            ...
            numericUpDown1.Enabled = hasChildren; // 0-6 yaş
            numericUpDown2.Enabled = hasChildren; // 7-18 yaş
            numericUpDown3.Enabled = hasChildren; // 18+ yaş
        }
        ...
        private void FormSalaryCalculator_Load(object sender, EventArgs e)
        {
            ...
            // NumericUpDown kontrollerinin başlangıç değerlerini ayarla
            numericUpDown1.Value = 0;
            numericUpDown2.Value = 0;
            numericUpDown3.Value = 0;
            
            // NumericUpDown kontrolleri varsayılan olarak devre dışı
